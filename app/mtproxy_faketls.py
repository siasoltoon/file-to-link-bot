...
class MTProxyFakeTLSClientCodec:
    ...
    def build_new_client_hello_packet(self) -> bytes:
        ...
        key_share = self._x25519_public_key()
        ...
        domain = self.domain
        sni = (b"\x00\x00" + struct.pack(">H", 5 + len(domain)) +
        ...
        extensions = b"".join((
            grease_ext + b"\x00\x00", sni, b"\x00\x17\x00\x00",
            b"\xff\x01\x00\x01\x00",
            b"\x00\x0a\x00\x0a\x00\x08" + grease_group + b"\x00\x1d\x00\x17\x00\x18",
            b"\x00\x0b\x00\x02\x01\x00", b"\x00\x23\x00\x00", alpn,
            ...
            b"\x00\x1b\x00\x03\x02\x00\x02", grease_tail + b"\x00\x01\x00"))
        body = (b"\x03\x03" + b"\x00" * 32 + b"\x20" + self.session_id +
                struct.pack(">H", len(ciphers)) + ciphers + b"\x01\x00" +
                struct.pack(">H", len(extensions)) + extensions)
        padding = 508 - len(body)
        if padding < 0:
            raise ValueError("MTProxy Fake-TLS ClientHello is too large")
        body += b"\x00\x15" + struct.pack(">H", padding) + b"\x00" * padding
        hello = (b"\x16\x03\x01" + struct.pack(">H", 4 + len(body)) + b"\x01" +
                 len(body).to_bytes(3, "big") + body)
        digest = _hmac_sha256(self.secret, hello)
        fake_time = int(time.time()).to_bytes(4, "little")
        self.random = digest[:28] + bytes(a ^ b for a, b in zip(fake_time, digest[28:32]))
        return hello[:11] + self.random + hello[43:]
...
class ConnectionTcpMTProxyFakeTLS(ConnectionTcpMTProxyRandomizedIntermediate):
    """Connect to MTProxy using Fake-TLS first, then Telethon MTProto."""
    ...
    async def _connect(self, timeout=None, ssl=None):
        ...
        self._reader, self._writer = await asyncio.wait_for(
            asyncio.open_connection(host=self._ip, port=self._port, ssl=ssl, local_addr=local_addr),
            timeout=timeout)

        raw_reader, raw_writer = self._reader, self._writer
        raw_writer.write(self.fake_tls_codec.build_new_client_hello_packet())
        await raw_writer.drain()
        server_hello = await self._read_server_hello(raw_reader)
        if not self.fake_tls_codec.verify_server_hello(server_hello):
            raise ConnectionError("MTProxy Fake-TLS server hello verification failed")
    ...
    @staticmethod
    async def _read_server_hello(reader):
        head = await reader.readexactly(133)
